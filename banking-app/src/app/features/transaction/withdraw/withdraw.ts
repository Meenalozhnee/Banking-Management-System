...
@Component({
  ...
  template: `
    <div class="page">
      <div class="page-header">
        ...
        <p class="subtitle">Withdraw money from your account</p>
      </div>
      <div class="form-card">
        <div class="form-icon"><i class="fas fa-arrow-up"></i></div>
        <form (ngSubmit)="onSubmit()" #wdForm="ngForm">
          ...
          <div class="input-group">
            ...
            <input type="number" name="amount" [(ngModel)]="amount" required min="1" placeholder="Enter withdrawal amount"/>
          </div>
          <div class="input-group">
            <label><i class="fas fa-align-left"></i> Description (Optional)</label>
            <input type="text" name="description" [(ngModel)]="description" placeholder="e.g. ATM withdrawal"/>
          </div>
          <div class="form-actions">
            <button type="submit" class="btn-primary" [disabled]="!wdForm.form.valid">
              <i class="fas fa-check-circle"></i> Withdraw
            </button>
          </div>
        </form>
      </div>
  ...
  styles: [`
    ...
    .form-icon { width: 56px; height: 56px; border-radius: 50%; background: linear-gradient(135deg, #fbe9e7, #ffccbc); display: flex; align-items: center; justify-content: center; margin: -56px auto 24px; font-size: 24px; color: #c62828; }
...
export class WithdrawComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.transactionService.withdraw(this.selectedAccount.accountNumber, { amount: this.amount, description: this.description }).subscribe({
      next: () => {
        this.snackBar.open('Withdrawal successful!', 'Close', { duration: 4000 });
        this.amount = 0;
        this.description = '';
        this.state.emit(AppEvents.WITHDRAW_COMPLETED);
        setTimeout(() => { this.accountService.reloadAccounts(); }, 500);
        this.selectedAccount = null;
      },
      error: (err: any) => this.snackBar.open(err.error?.message || 'Withdrawal failed', 'Close', { duration: 5000 })
    });
  }
}
